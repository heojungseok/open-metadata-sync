#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
PROJECT_DIR=$(cd "$SCRIPT_DIR/.." && pwd)
cd "$PROJECT_DIR"

suffix="$$-$(date -u +%s)"
network="open-metadata-sync-live-db-test-$suffix"
volume="open-metadata-sync-live-db-test-$suffix"
mysql_container="open-metadata-sync-live-db-test-mysql-$suffix"
secret_dir=$(mktemp -d)
root_password=$(openssl rand -hex 32)
live_password=$(openssl rand -hex 32)
replay_password=$(openssl rand -hex 32)
printf '%s\n' "$root_password" > "$secret_dir/root"
printf '%s\n' "$live_password" > "$secret_dir/live"
chmod 600 "$secret_dir/root" "$secret_dir/live"

cleanup() {
  docker rm -f "$mysql_container" >/dev/null 2>&1 || true
  docker volume rm "$volume" >/dev/null 2>&1 || true
  docker network rm "$network" >/dev/null 2>&1 || true
  rm -rf "$secret_dir"
}
trap cleanup EXIT

docker network create "$network" >/dev/null
docker volume create "$volume" >/dev/null
docker run -d --name "$mysql_container" --network "$network" --network-alias mysql \
  -e MYSQL_ROOT_PASSWORD="$root_password" -v "$volume:/var/lib/mysql" \
  mysql:8.4.10@sha256:8dbcf531a03aade657e181b9cf2f1d1803ce621a1d55610cb44cb531ab7d7db6 >/dev/null
for _ in {1..60}; do
  if docker exec -e MYSQL_PWD="$root_password" "$mysql_container" \
      mysql --batch --skip-column-names -uroot -e "SELECT 1" >/dev/null 2>&1; then
    break
  fi
  sleep 1
done
docker exec -e MYSQL_PWD="$root_password" "$mysql_container" \
  mysql --batch --skip-column-names -uroot -e "SELECT 1" >/dev/null

docker exec -i -e MYSQL_PWD="$root_password" "$mysql_container" mysql -uroot <<SQL
CREATE DATABASE open_metadata;
CREATE USER 'open_metadata'@'%' IDENTIFIED BY '${replay_password}';
GRANT ALL PRIVILEGES ON open_metadata.* TO 'open_metadata'@'%';
CREATE TABLE open_metadata.replay_guard (id INT PRIMARY KEY);
SQL

for _ in 1 2; do
  docker run --rm --network "$network" --entrypoint /bin/bash \
    -e MYSQL_ROOT_PASSWORD_FILE=/run/secrets/root \
    -e LIVE_DB_PASSWORD_FILE=/run/secrets/live \
    -e DEMO_BOOTSTRAP_ACK=CREATE_LIVE_DEMO \
    -v "$secret_dir/root:/run/secrets/root:ro" \
    -v "$secret_dir/live:/run/secrets/live:ro" \
    -v "$PROJECT_DIR/scripts:/opt/demo/scripts:ro" \
    mysql:8.4.10@sha256:8dbcf531a03aade657e181b9cf2f1d1803ce621a1d55610cb44cb531ab7d7db6 \
    /opt/demo/scripts/demo-bootstrap-live-db.sh
done

docker exec -i -e MYSQL_PWD="$root_password" "$mysql_container" mysql -uroot <<'SQL'
CREATE TABLE open_metadata_live_demo.live_guard (id INT PRIMARY KEY);
SQL

expect_denied() {
  local username=$1
  local password=$2
  local statement=$3
  if docker run --rm --network "$network" -e MYSQL_PWD="$password" \
      mysql:8.4.10@sha256:8dbcf531a03aade657e181b9cf2f1d1803ce621a1d55610cb44cb531ab7d7db6 \
      mysql --protocol=TCP -hmysql -P3306 -u"$username" -e "$statement" >/dev/null 2>&1; then
    echo "Unexpected cross-schema privilege: $username: $statement" >&2
    exit 1
  fi
}

expect_denied open_metadata_live_demo "$live_password" "SELECT * FROM open_metadata.replay_guard"
expect_denied open_metadata_live_demo "$live_password" "TRUNCATE TABLE open_metadata.replay_guard"
expect_denied open_metadata_live_demo "$live_password" "CREATE TABLE open_metadata.forbidden (id INT)"
expect_denied open_metadata_live_demo "$live_password" "DROP DATABASE open_metadata"
expect_denied open_metadata "$replay_password" "SELECT * FROM open_metadata_live_demo.live_guard"
expect_denied open_metadata "$replay_password" "TRUNCATE TABLE open_metadata_live_demo.live_guard"
expect_denied open_metadata "$replay_password" "CREATE TABLE open_metadata_live_demo.forbidden (id INT)"
expect_denied open_metadata "$replay_password" "DROP DATABASE open_metadata_live_demo"

docker run --rm --network "$network" -e MYSQL_PWD="$live_password" \
  mysql:8.4.10@sha256:8dbcf531a03aade657e181b9cf2f1d1803ce621a1d55610cb44cb531ab7d7db6 \
  mysql --protocol=TCP -hmysql -P3306 -uopen_metadata_live_demo \
  -e "SELECT * FROM open_metadata_live_demo.live_guard" >/dev/null
echo "Live/replay database privilege isolation passed"
